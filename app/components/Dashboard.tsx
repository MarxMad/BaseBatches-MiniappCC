...
export const Dashboard = () => {
  ...
  const handleGameEnd = async (score: number, duration: number, result: 'win' | 'lose' | 'completed') => {
    ...
    if (user?.fid) {
      try {
        ...
        const gameName = gameNames[selectedGame || ''] || 'Juego';
        const message = result === 'win' 
          ? `¡Felicidades! Has ganado en ${gameName} con ${score} puntos en ${duration.toFixed(1)} segundos!`
          : `Has completado ${gameName} con ${score} puntos en ${duration.toFixed(1)} segundos.`;

        await fetch('/api/webhook/send-notification', {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            fid: user.fid,
            notificationId: `game-complete-${selectedGame}`,
            title: `¡${gameName} Completado! 🎮`,
            body: message
          }),
        });
      } catch (error) {
  ...
  return (
    <div className="space-y-8">
      <h2 className="text-3xl font-bold text-white mb-6">Juegos Educativos</h2>
      
      {selectedGame ? (
        <div className="relative">
          {selectedGame === 'catch-coin' && (
            <CatchTheCoin 
      ...
      {gameStats.length > 0 && (
        <div className="mt-8">
          ...
          <GameStats stats={gameStats} onClose={() => {}} />
        </div>
      )}
    </div>
  );
